...
String ezButtons::poll() {
	...
	#ifdef M5EZ_FACES
		if (ez.faces.on() && keystr == "") keystr = ez.faces.poll();
	#endif
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				case 2:
					{
						...
						while (true) {
							if (!_inactivity) {
								...
								disp_val = "Backlight will turn off after " + String(_inactivity * 5) + " seconds inactivity";
							}
	...
	uint16_t ezBacklight::loop() {
		if (!_backlight_off && _inactivity) {
			if (millis() > _last_activity + 5000 * _inactivity) {
				_backlight_off = true;
...
*/
		...
		if(digitalRead(KEYBOARD_INT) == LOW) {
			...
			while(Wire.available()) { 
				...
				switch(key) {
					case 1: out += "up"; break; // DPAD UP
					case 2: out += "down"; break; // DPAD DOWN
					case 4: out += "left"; break; // DPAD LEFT
					case 8: out += "right"; break; // DPAD RIGHT
					case 0x10: out += "A"; break; // A
					case 0x20: out += "B"; break; // B
					case 0x40: out += "select"; break; // SELECT
					case 0x80: out += "start"; break; // START
				}
			}
#ifdef M5EZ_BACKLIGHT
			ez.backlight.activity();
...
int16_t ezMenu::_runTextOnce() {
	...
	while (true) {
		...
		while (true) {
			...
			if (pressed != "" && pressed != "left" && pressed != "right") break;
		}
		if (pressed == "up") {
			if (--_selected < 0) _selected = _items.size() - 1;
			_fixOffset();
		} else if (pressed == "down") {
			if (++_selected >= _items.size()) _selected = 0;
			_fixOffset();
